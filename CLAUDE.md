...
- **StatRow giữ đúng 3 cột** (theo ảnh mẫu, component đã dựng sẵn từ trước) dù mục 4.7b liệt kê 4 số liệu cho Admin (nhân sự/lớp đang mở/slot trống/cảnh báo pool nhỏ) — "cảnh báo pool nhỏ" gộp thành badge cạnh tiêu đề thẻ "Việc cần duyệt" thay vì thêm thẻ stat thứ 4.
...
- **Bảng việc cần duyệt** gộp đăng ký tự do đang chờ (`getViecCuaToi(uid, true).can_duyet`) + đề xuất nhân sự đang chờ (`getDeXuatList("cho_duyet")`), sắp theo mới nhất, rút gọn 8 dòng, nút hành động nhanh ngay trong bảng (`NutDuyet`, `DuyetButtons` — tái dùng nguyên component đã có ở `/dang-ky` và `/nhan-su/de-xuat`).
- **Lịch dạy sắp tới** = timeline dọc (không phải mini-calendar lưới) nhóm theo "Hôm nay/Ngày mai/Tuần này/ngày cụ thể" — đúng tinh thần mục 8.9 (mobile ưu tiên nhóm theo ngày hơn lưới calendar thu nhỏ), dùng chung 1 cách hiển thị cho mọi cỡ màn hình thay vì làm 2 layout riêng.
- **Kiểm thử:** không migration nên không cần chạy SQL Editor; đã viết script service-role chỉ-đọc kiểm tra cấu trúc truy vấn mới (tên bảng/cột/quan hệ join) chạy đúng trên Supabase thật rồi xóa ngay — script này KHÔNG kiểm tra được RLS theo vai trò (service role bỏ qua RLS), nên người dùng cần tự đăng nhập bằng cả tài khoản Admin thật lẫn tài khoản GV/TG để xác nhận hiển thị đúng trước khi coi lượt 3 là hoàn tất.
