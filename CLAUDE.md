...
- **Giảng viên/Trợ giảng:**
  - **Lịch dạy sắp tới** dạng timeline GỌN (tối đa 4 Bài chưa diễn ra, `LichSapToi`) kèm link "Xem lịch tháng"; lịch tháng đầy đủ (`LichThang`, nhiều màu theo lớp trong 4 màu gốc, bấm ngày xem Bài của ngày) đặt ở đầu trang **Đăng ký giảng dạy** (`/dang-ky#lich`) — icon lịch topbar và tab mobile "Lịch dạy" đều trỏ về đó; chỉ Bài đã phân công, không có lớp đang chờ/slot trống
...
- **Lịch dạy (điều chỉnh sau phản hồi):** Tổng quan giữ timeline gọn 4 Bài + link; lưới tháng `LichThang` (mỗi ô ngày tô gradient theo màu lớp, chấm theo số Bài, bấm ngày → danh sách Bài) ở `/dang-ky#lich`. Dữ liệu `getLichCuaToi()` lấy 6 tháng trước → 12 tháng sau một lần, chuyển tháng phía client, không cần migration; `isActive` bỏ phần `#` khi so đường dẫn.
