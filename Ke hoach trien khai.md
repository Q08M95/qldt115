...
> Chỉnh theo phản hồi test lần 2 (`20260928120000`, test `giai_doan_8e_lien_ket.sql` 5 kiểm tra): thông báo cần duyệt/mời dạy dẫn tới `/dang-ky`, dọn thông báo đã đọc sau 90 ngày, danh sách chia nhóm theo thời gian; `/danh-gia` thay cột Lịch sử giảng dạy bằng 2 thẻ KPI (A/B/C qua các kỳ, từng tiêu chí so với kỳ trước). Đã chạy migration + test 5/5 true trên Supabase thật.
> Chỉnh theo phản hồi test (`20260928110000`, test `giai_doan_8d_giai_quyet_thong_bao.sql` 5 kiểm tra): thông báo cần hành động không tự đã đọc khi bấm xem; job dọn thông báo hết hạn xử lý; gom cài đặt thông báo vào 1 thẻ thu gọn; banner check-in gọn và chỉ ở Trang chủ; `/danh-gia` thêm cột Lịch sử giảng dạy. Đã chạy migration + test 5/5 true trên Supabase thật.
> Gộp bớt thông báo (`20260928100000`, test `giai_doan_8c_gop_thong_bao.sql` 12 kiểm tra): đăng ký cần duyệt gộp theo lớp, nhắc check-in gộp Bài liền nhau (< 3 giờ). Đã chạy migration + test 12/12 true trên Supabase thật.
> Bổ sung theo đề xuất rà soát (`20260927100000`, `20260927110000`): tùy chọn thông báo theo loại/kênh, thử lại push, nhật ký ghi thêm tạo/xóa lớp + thêm/sửa/xóa Bài + tạo đề xuất, xuất Excel nhật ký. Đã chạy migration + test `giai_doan_8b_9c_bo_sung.sql` (18/18 true) trên Supabase thật; **còn cần thử nhận push trên điện thoại thật**.
...
> **Tiến độ (chia 3 lượt):** lượt 1 = báo cáo #3, #4, #5, #8 (khung tuần/tháng/quý/năm) — đã viết xong migration `20260929100000_giai_doan_10a_bao_cao.sql` + test `supabase/tests/giai_doan_10a_bao_cao.sql` (26 kiểm tra, PGlite cục bộ 26/26; hồi quy Giai đoạn 3-8, 9 vẫn pass) + trang `/bao-cao` (4 báo cáo, có demo dev `/design/bao-cao`); **đã xác nhận trên Supabase thật: migration + test 26/26 true, đã push**. Lượt 2 = báo cáo #1, #2, #6, #7 (theo kỳ) + xuất Excel cho #1, #3, #6, #7 — đã viết xong migration `20260930100000_giai_doan_10b_bao_cao_theo_ky.sql` + test `supabase/tests/giai_doan_10b_bao_cao_ky.sql` (20 kiểm tra, PGlite cục bộ 20/20; hồi quy Giai đoạn 3-9, 10a vẫn pass) + trang `/bao-cao` (đủ 8 báo cáo) + route `/bao-cao/xuat`; **chờ người dùng chạy migration + test trên Supabase thật rồi mới push**. Lượt 3 = trang Tổng quan Admin + GV/TG.
