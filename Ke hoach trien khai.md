...
> Chỉnh theo phản hồi test (`20260928110000`, test `giai_doan_8d_giai_quyet_thong_bao.sql` 5 kiểm tra): thông báo cần hành động không tự đã đọc khi bấm xem; job dọn thông báo hết hạn xử lý; gom cài đặt thông báo vào 1 thẻ thu gọn; banner check-in gọn và chỉ ở Trang chủ; `/danh-gia` thêm cột Lịch sử giảng dạy. Chờ chạy migration + test trên Supabase thật.
> Gộp bớt thông báo (`20260928100000`, test `giai_doan_8c_gop_thong_bao.sql` 12 kiểm tra): đăng ký cần duyệt gộp theo lớp, nhắc check-in gộp Bài liền nhau (< 3 giờ). Chờ chạy migration + test trên Supabase thật.
> Bổ sung theo đề xuất rà soát (`20260927100000`, `20260927110000`): tùy chọn thông báo theo loại/kênh, thử lại push, nhật ký ghi thêm tạo/xóa lớp + thêm/sửa/xóa Bài + tạo đề xuất, xuất Excel nhật ký. Chờ chạy migration + test `giai_doan_8b_9c_bo_sung.sql` (18 kiểm tra) trên Supabase thật; **cần thử nhận push trên điện thoại thật**.
