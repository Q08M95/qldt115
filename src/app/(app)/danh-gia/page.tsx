...
import { getLichSuGiangDay } from "@/lib/nhan-su/queries";

// Đánh giá chất lượng (mục 4.4): Bảng KPI cá nhân của chính mình. KPI của người khác xem từ hồ sơ nhân sự.
// Banner check-in chỉ ở Trang chủ (không lặp lại ở đây). Bố cục 2 cột như hồ sơ nhân sự: KPI (trái) | Lịch sử giảng dạy của tôi (phải).
export default async function DanhGiaPage() {
  const { profile } = await requireSession();
  const [kpi, lichSu] = await Promise.all([getKpiCaNhan(profile.id), getLichSuGiangDay(profile.id)]);
